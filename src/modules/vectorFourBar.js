...
class VectorFourBar {
  ...
  couplerAngle(input, coupler, output, ground, thetaTwo) {
    ...
    this.linkage.thetaThree = this.modQuadratic(
      this.constants.D,
      this.constants.E,
      this.constants.F
    );
  ...
  outputAngle(input, coupler, output, ground, thetaTwo) {
    ...
    this.linkage.thetaFour = this.modQuadratic(
      this.constants.A,
      this.constants.B,
      this.constants.C
    );
  ...
  couplerVector(input, coupler, output, ground, thetaTwo, delta) {
    ...
    this.linkage.vectorCoupler.open.Re =
      coupler * Math.cos(this.linkage.thetaThree.open + delta);
    this.linkage.vectorCoupler.open.Im =
      coupler * Math.sin(this.linkage.thetaThree.open + delta);

    this.linkage.vectorCoupler.crossed.Re =
      coupler * Math.cos(this.linkage.thetaThree.crossed + delta);
    this.linkage.vectorCoupler.crossed.Im =
      coupler * Math.sin(this.linkage.thetaThree.crossed + delta);
  ...
  outputVector(input, coupler, output, ground, thetaTwo, delta) {
    ...
    this.linkage.vectorOutput.open.Re =
      coupler * Math.cos(this.linkage.thetaFour.open + delta);
    this.linkage.vectorOutput.open.Im =
      coupler * Math.sin(this.linkage.thetaFour.open + delta);

    this.linkage.vectorOutput.crossed.Re =
      coupler * Math.cos(this.linkage.thetaFour.crossed + delta);
    this.linkage.vectorOutput.crossed.Im =
      coupler * Math.sin(this.linkage.thetaFour.crossed + delta);
  ...
  transmissionAngle(input, coupler, output, ground, thetaTwo) {
    ...
    if (
      typeof this.linkage.thetaThree != "string" &&
      typeof this.linkage.thetaFour != "string"
    ) {
      transmissionAngles.open = Math.abs(
        this.linkage.thetaThree.open - this.linkage.thetaFour.open
      );
      transmissionAngles.crossed = Math.abs(
        this.linkage.thetaThree.crossed - this.linkage.thetaFour.crossed
      );
  ...
  getConstantsK() {
    ...
    this.constants.K3 =
      (Math.pow(this.linkage.input, 2) -
        Math.pow(this.linkage.coupler, 2) +
        Math.pow(this.linkage.output, 2) +
        Math.pow(this.linkage.ground, 2)) /
      (2 * this.linkage.input * this.linkage.output);
    ...
    this.constants.K5 =
      (Math.pow(this.linkage.output, 2) -
        ...
        Math.pow(this.linkage.input, 2) -
        Math.pow(this.linkage.coupler, 2)) /
      (2 * this.linkage.input * this.linkage.coupler);
  }

  getConstantsTierA() {
    this.constants.A =
      Math.cos(this.linkage.thetaTwo) -
      this.constants.K1 -
      this.constants.K2 * Math.cos(this.linkage.thetaTwo) +
      this.constants.K3; // Radians
    this.constants.B = -2 * Math.sin(this.linkage.thetaTwo);
    this.constants.C =
      this.constants.K1 -
      (this.constants.K2 + 1) * Math.cos(this.linkage.thetaTwo) +
      this.constants.K3;
  }
  ...
  getConstantsTierD() {
    this.constants.D =
      Math.cos(this.linkage.thetaTwo) -
      this.constants.K1 +
      this.constants.K4 * Math.cos(this.linkage.thetaTwo) +
      this.constants.K5; // Radians
    this.constants.E = -2 * Math.sin(this.linkage.thetaTwo);
    this.constants.F =
      this.constants.K1 +
      (this.constants.K4 - 1) * Math.cos(this.linkage.thetaTwo) +
      this.constants.K5;
  }
  ...
  modQuadratic(A, B, C) {
    ...
    var temp = {};
    temp.crossed =
      2 * Math.atan((-B + Math.sqrt(Math.pow(B, 2) - 4 * A * C)) / (2 * A));
    temp.open =
      2 * Math.atan((-B - Math.sqrt(Math.pow(B, 2) - 4 * A * C)) / (2 * A));
  ...
  errorCheck() {
    if (
      !this.linkage.input ||
      !this.linkage.coupler ||
      !this.linkage.output ||
      !this.linkage.ground ||
      !this.linkage.thetaTwo
    ) {
      console.log(
        "Must Provide Linkage Lengths and Input Angle for a Solution"
      );
      return false;
    } else if (
